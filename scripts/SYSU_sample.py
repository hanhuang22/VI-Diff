...
from guided_diffusion.image_datasets import SYSUModFolder, SYSUFolder
...
def main():
    ...
    if args.l_cls_id != 0:
        args.dataset='sysu_rgb'
        classifier_id = create_classifier(**args_to_dict(args, classifier_defaults().keys()))
        classifier_id.load_state_dict(
            dist_util.load_state_dict(args.cls_id_path, map_location="cpu")
        )
        classifier_id.to(dist_util.dev())
        classifier_id.eval()
    if args.l_cls_mod != 0:
        args.dataset='sysu'
        classifier_mod = create_classifier(**args_to_dict(args, classifier_defaults().keys()))
        classifier_mod.load_state_dict(
            dist_util.load_state_dict(args.cls_mod_path, map_location="cpu")
        )
        classifier_mod.to(dist_util.dev())
        classifier_mod.eval()
